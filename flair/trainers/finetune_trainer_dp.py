...
class ModelFinetuner_dp(ModelDistiller):
	def __init__(
		self,
		...
		for corpus_name in self.corpus2id:	# {'CONLL_03_DP_TEST': 0}
			...
			if down_sample_amount>0:
				if len(self.corpus.train_list[i].sentences)>down_sample_amount:
					# no down_sample_amount under 3k unlabeled data.
	...
	def train(
		...
		**kwargs,
		...
		if self.distill_mode:	
			...
			elif self.model.distill_dp:
				t1_aptt = time.time()
				train_data = self.assign_pretrained_teacher_targets(coupled_train_data,self.teachers,best_k=best_k)
				t2_aptt = time.time()
				print('assign_pretrained_teacher_targets time is:', datetime.timedelta(seconds=t2_aptt-t1_aptt))
			else:
		...
		if self.distill_mode:
			...
			# 200830 ner dp
			t1_resort = time.time()
			batch_loader = self.resort(batch_loader, is_crf=self.model.distill_crf, is_posterior = self.model.distill_dp, is_token_att = self.model.token_level_attention)	# now we can use batch.teacher_features['posteriors'] 
			t2_resort = time.time()
			print('resort time is:', datetime.timedelta(seconds=t2_resort-t1_resort))
		...
		try:	# ner dp
			...
			for epoch in range(0 + self.epoch, max_epochs + self.epoch):
				...
				#modified as github at 200826
				if self.distill_mode:
					if self.teacher_annealing:
						interpolation=max(1-(((epoch-warmup_bias)*total_number_of_batches)/total_number_of_batches*self.anneal_factor)/100.0, 0)
					else:
	...
	def assign_pretrained_teacher_targets(self,coupled_train_data,teachers,best_k=10):
		...
		for teacherid, teacher in enumerate(teachers):
			...
			for index, train_data in enumerate(coupled_train_data):
				...
				if target not in teacher.targets:
					continue
				loader=ColumnDataLoader(list(train_data),self.mini_batch_size,grouped_data=False,use_bert=use_bert, sentence_level_batch=self.sentence_level_batch)
				loader.word_map = teacher.word_map
				loader.char_map = teacher.char_map
				...
				for batch in loader:
					...
					with torch.no_grad():
						...
						if self.model.tag_type=='dependency':
							...
							arc_scores, rel_scores=teacher.forward(teacher_input)
							logits = arc_scores
						elif teacher.tag_type == 'ner_dp':
							_, logits = teacher.forward(teacher_input)
						else:
							logits=teacher.forward(teacher_input)
						...
						if self.model.distill_dp:

							tx1 = time.time()
						
							bioes_scores = teacher.distill_dp_transform(batch, logits, self.model.tag_dictionary, self.model.temperature)
							# bioes_scores1 = teacher.distill_dp_transform_lse(batch, logits, self.model.tag_dictionary, self.model.temperature)
							# delta_probs += (bioes_scores - bioes_scores1).sum()/len(batch)
							n_batches += len(batch)
							tx2 = time.time()
							dt = tx2-tx1
							tt += dt
					...
					for idx, sentence in enumerate(teacher_input):
						...
						if self.model.distill_dp:
							sentence.set_teacher_posteriors(bioes_scores[idx], self.embeddings_storage_mode)

						teacher_input[idx].clear_embeddings()
					del logits
				
				
				print('set_teacher_posterior time is:', datetime.timedelta(seconds=tt))
				print('number of batches:{}'.format(n_batches))
				print('delta probs:{}'.format(delta_probs))
				store_embeddings(teacher_input, "none")
			teacher = teacher.to('cpu')
			# del teacher
	...
	def resort(self,loader,is_crf=False, is_posterior=False, is_token_att=False):
		for batch in loader:
			if is_posterior:
				try:
					posteriors=[x._teacher_posteriors for x in batch]	#len(posterior) = n_sentences, posterior[sent_idx] = list of posterior vector
					posterior_lens=[len(x[0]) for x in posteriors]
					lens=posterior_lens.copy()	# list of sentence length.
