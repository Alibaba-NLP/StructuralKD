...
class FastWordEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, embeddings: str, all_tokens: list, field: str = None, if_cased: bool = True, freeze: bool = False, additional_empty_embedding: bool = False, keepall: bool = False, normalize = False):
        """
        Initializes classic word embeddings. Constructor downloads required files if not there.
        :param embeddings: one of: 'glove', 'extvec', 'crawl' or two-letter language code or custom
        If you want to use a custom embedding file, just pass the path to the embeddings as embeddings variable.
        ...
        if normalize:
            for i in range(embeddings_tmp.shape[0]):
                norm = torch.norm(embeddings_tmp[i])
                if norm > 0:
                    embeddings_tmp[i] = embeddings_tmp[i]/norm
