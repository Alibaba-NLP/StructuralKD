...
class ModelDistiller(ModelTrainer):
	def __init__(
		...
		**kwargsm
		...
		if self.distill_mode:
			self.teacher_config = config.teacher_config
			self.corpus_teacher: ListCorpus = self.teacher_config.corpus
	...
	def train(
		...
		**kwargs,
		...
		if self.distill_mode:
			...
			batch_loader=ColumnDataLoader(train_data,mini_batch_size,shuffle,use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map)
		else:
			batch_loader=ColumnDataLoader(ConcatDataset(train_data),mini_batch_size,shuffle,use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map)
		batch_loader.assign_tags(self.model.tag_type,self.model.tag_dictionary)
		if self.distill_mode:
			if faster:
				batch_loader=self.resort(batch_loader,is_crf=self.model.distill_crf, is_posterior = self.model.distill_posterior)

		dev_loader=ColumnDataLoader(list(self.corpus.dev),eval_mini_batch_size,use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map)
		dev_loader.assign_tags(self.model.tag_type,self.model.tag_dictionary)
		test_loader=ColumnDataLoader(list(self.corpus.test),eval_mini_batch_size,use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map)
		...
		try:
			...
			for epoch in range(0 + self.epoch, max_epochs + self.epoch):
				...
				if log_train:
					...
					result_line += f"\t{train_loss}\t{train_eval_result.log_line}"
					log.info(
	...
	def assign_pretrained_teacher_targets(self,coupled_train_data,teachers,best_k=10):
		...
		for teacher in teachers:
			...
			for index, train_data in enumerate(coupled_train_data):
				...
				# if target not in teacher.targets:
				# 	continue
				loader=ColumnDataLoader(list(train_data),self.mini_batch_size,grouped_data=True,use_bert=use_bert,word_map = teacher.word_map, char_map = teacher.char_map)
				...
				# pdb.set_trace()
				teacher_batch_loader = loader.assign_tags(teacher.tag_type, teacher.tag_dictionary, teacher_input=teacher_inputs)
				# if self.model.tag_dictionary.item2idx !=teachers[0].tag_dictionary.item2idx:
				# 	pdb.set_trace()
				...
				for num, batch in enumerate(loader):
					...
					student_input, teacher_input = zip(*batch)
					student_input=list(student_input)
					teacher_input=teacher_batch_loader[num]
					lengths1 = torch.Tensor([len(sentence.tokens)-1 for sentence in teacher_input])
					lengths2 = torch.Tensor([len(sentence.tokens) for sentence in student_input])
					# pdb.set_trace()
					assert (lengths1==lengths2).all(), 'two batches are not equal!'
					max_len = max(lengths1)
					...
					with torch.no_grad():
						if 'PTB_SL' in self.corpus.targets:
							arc_score, rel_score = teacher.forward(teacher_input)
						else:
							logits=teacher.forward(teacher_input)
						...
						if self.model.distill_posterior:
							if 'PTB_SL' in self.corpus.targets:
								arc_prob = torch.softmax(arc_score,-1)
								rel_prob = torch.softmax(rel_score,-1)
								# pdb.set_trace()
								# n x m x len(self.dictionary)
								teacher_distribution = self.covert_teacher_prob(arc_prob, rel_prob, teacher.tag_dictionary.item2idx, self.model.tag_dictionary.item2idx)
							else:
						...
						for idx, sentence in enumerate(student_input):
							...
							if self.model.distill_posterior:
								if 'PTB_SL' in self.corpus.targets:
									# pdb.set_trace()
									sentence.set_teacher_posteriors(teacher_distribution[idx], self.embeddings_storage_mode)
								else:
									sentence.set_teacher_posteriors(forward_backward_score[idx], self.embeddings_storage_mode)
							teacher_input[idx].clear_embeddings()
						del arc_score, rel_score
	...
	def final_test(
		...
		if overall_test:
			loader=ColumnDataLoader(list(self.corpus.test),eval_mini_batch_size, use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map,sort_data=False)
		...
		'''
			...
			for index,subcorpus in enumerate(self.corpus.test_list):
				log_line(log)
				log.info('current corpus: '+self.corpus.targets[index])
				loader=ColumnDataLoader(list(subcorpus),eval_mini_batch_size,use_bert=self.use_bert,word_map = self.model.word_map, char_map = self.model.char_map)
				loader.assign_tags(self.model.tag_type,self.model.tag_dictionary)
				current_result, test_loss = self.model.evaluate(
					loader,
					out_path=base_path / f"{self.corpus.targets[index]}-test.tsv",
	...
	def covert_teacher_prob(self, arc_mat, rel_mat, rel_set, label_set):
		"""
		:param arc_mat: arc probability from biaffine parser, n x m(child) x m(head)
		:param rel_mat: relation probability from biaffine parser, n x m(child) x m(head) x len(relation)
		:param rel_set: relation set from biaffine parser, item2idx, a Dict
		:param label_set: label set from sequence labeling dependency parser, item2idx
		:return: probability distribution in label set converted from biaffine parser, n x m x len(label_set)
		"""
		tensor_shape = list(arc_mat.size())
		batch_size, sent_len = tensor_shape[0], tensor_shape[1]

		convert_dist = []
		for token_idx in range(1, sent_len):
			label_dist = torch.zeros(batch_size, len(label_set)).to(flair.device)  # n x l
			label_list = []
			index_list = []
			for label in label_set:
				if b'_' in label:
					head_bias, rel = label.split(b'_')
					if token_idx + int(head_bias) < 0 or token_idx + int(head_bias) > sent_len - 1:
						continue
					else:
						head_idx = token_idx + int(head_bias)
						rel_idx = rel_set[rel]
						sel_arc_prob = arc_mat[:, token_idx, head_idx]  # n x 1
						sel_rel_prob = rel_mat[:, token_idx, head_idx, rel_idx]  # n x 1
						label_list.append(sel_arc_prob * sel_rel_prob)
						index_list.append(label_set[label])  # 1
			# pdb.set_trace()
			label_dist_tensor = torch.stack(label_list, dim=1)  # n x ?
			# label_dist_tensor = torch.softmax(label_dist_tensor, 1)  # n x ?
			label_dist = label_dist.to(label_dist_tensor.dtype)
			label_dist = label_dist.scatter_(1, torch.stack([torch.LongTensor(index_list)] * batch_size, 0).to(flair.device),label_dist_tensor)
			label_dist = torch.softmax(torch.log(label_dist + 1e-13) / self.model.temperature,1)
			convert_dist.append(label_dist)  # m number of n x l
		convert_dist_tensor = torch.stack(convert_dist, dim=0)  # m x n x l
		convert_dist_tensor = convert_dist_tensor.permute(1, 0, 2)
		# pdb.set_trace()
		return convert_dist_tensor