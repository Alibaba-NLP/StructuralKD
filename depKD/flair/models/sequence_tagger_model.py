...
class SequenceTagger(flair.nn.Model):
	def __init__(
		...
		self.lstm_dropout_out: float = lstm_dropout_out
		...
		if self.use_rnn:
			...
			if self.rnn_type in ["LSTM", "GRU"]:
				...
				self.rnn = getattr(torch.nn, self.rnn_type)(
					...
					dropout=0.0 if self.nlayers == 1 else self.lstm_dropout_out,
					bidirectional=True,
				)
	...
	def simple_forward_distillation_loss(
		...
		interpolation=0.05, train_with_professor=False, professor_interpolation=0.5,
	) -> torch.tensor:
		...
		if self.distill_crf:
			...
			for s_id, sentence in enumerate(data_points):
				...
				if train_with_professor:
					...
					#===============================For dependency parsing as sequence labeling=================================================
					try:
						teacher_features[s_id][: len(sentence)] = sentence.get_teacher_sl_posteriors()
					except:
						pdb.set_trace()
					#===============================For dependency parsing as sequence labeling=================================================
				# pdb.set_trace()
	...
	def _calculate_distillation_loss(self, features, teacher_features, lengths, T = 1., teacher_is_score=False):
		...
		mask=self.mask
		if not teacher_is_score:
			KD_loss = torch.nn.functional.kl_div(F.log_softmax(features/T, dim=-1), teacher_features,reduction='none').sum(-1) * mask * T * T
		else:
	...
	def _calculate_loss(
		self, features: torch.tensor, sentences: List[Sentence], mask: torch.tensor = None,
	) -> float:
	...
	def _obtain_labels(
		self, feature, sentences, get_all_tags: bool = True
...
# =================================For dependency parsing as sequence labeling==============================
class DependencySequenceTagger(SequenceTagger):
	...
	def _init_model_with_state_dict(state):
		...
		use_cnn = state["use_cnn"] if 'use_cnn' in state else False
		model = DependencySequenceTagger(
			hidden_size=state["hidden_size"],
			embeddings=state["embeddings"],
			tag_dictionary=state["tag_dictionary"],
			tag_type=state["tag_type"],
	...
	def simple_forward_distillation_loss(
		...
		longest_token_sequence_in_batch: int = max(lengths)
		features = self.forward(data_points)
		target_loss = self._calculate_loss(features, data_points, self.mask)
		if self.distill_crf:
			teacher_tags=torch.stack([sentence.get_teacher_target() for sentence in data_points],0)
			# proprocess, convert k best to batch wise
			seq_len=teacher_tags.shape[1]
			...
			for s_id, sentence in enumerate(data_points):
				...
				if train_with_professor:
					...
					#===============================For dependency parsing as sequence labeling=================================================
				# pdb.set_trace()
			distillation_loss = self._calculate_distillation_loss(features, teacher_features, self.mask, T=self.temperature,
			                                                      teacher_is_score=not self.distill_prob)
			return interpolation * distillation_loss + (1-interpolation) * target_loss
	...
	def _calculate_distillation_loss(self, features, teacher_features, mask, T=1, teacher_is_score=True):
		...
		if teacher_is_score:
			...
			teacher_prob = teacher_features
		KD_loss = torch.nn.functional.kl_div(F.log_softmax(features / T, dim=-1), teacher_prob,
	...
	def _calculate_loss(
		...
		tag_list: List = []
		try:
			tag_list = torch.stack(
				[getattr(sentence, self.tag_type + '_tags').to(flair.device) for sentence in sentences], 0).long()
		except:
			tag_list: List = []
			for s_id, sentence in enumerate(sentences):
				# get the tags in this sentence
		...
		if self.use_crf:
			...
			score = torch.nn.functional.cross_entropy(features.view(-1, features.shape[-1]), tag_list.view(-1, ),
			                                          reduction='none') * mask.view(-1, )
			_, predict = torch.max(features,2,keepdim=True)
			setattr(sentences, self.tag_type +'predict', predict)
			if self.sentence_level_loss or self.use_crf:
				score = score.sum() / features.shape[0]
			else:
				score = score.sum() / mask.sum()
	...
	def _score_sentence(self, feats, tags, lens_, mask=None):
		...
		transition_mask2[:, :-1] = mask
		transition_mask2[:, -1] = 0
		pad_stop_tags = pad_stop_tags.cuda() * transition_mask2.long() + (
					1 - transition_mask2.long()) * self.tag_dictionary.get_idx_for_item(STOP_TAG)
		...
		ts_energy = self.transitions.unsqueeze(0).unsqueeze(0).expand(bat_size, seq_len, self.tagset_size,
		                                                              self.tagset_size)

		# extract the first dimension (2nd dimension here) of transition scores
		# (bat_size, seq_len + 1, target_size_to, target_size_from) -> (bat_size, seq_len + 1, 1, target_size_from)
		ts_energy = torch.gather(ts_energy, 2, pad_stop_tags.unsqueeze(-1).unsqueeze(-1).expand(bat_size, seq_len, 1,
		                                                                                        feats.shape[-1]))
		# (bat_size, seq_len + 1, 1, target_size_from) -> (bat_size, seq_len + 1, target_size_from)
		ts_energy = ts_energy.squeeze(2)
		# (bat_size, seq_len + 1, target_size_from) -> (bat_size, seq_len + 1)
		...
		with torch.no_grad():
			...
			for batch in data_loader:
				...
				with torch.no_grad():
					...
					mask = self.mask
					loss = self._calculate_loss(features, batch, mask)
					tags, nbest_tags = self._obtain_labels(features, batch)
				...
				if post_process:
					for (sentence, sent_tags) in zip(batch, nbest_tags):
						for (token, tags) in zip(sentence.tokens, sent_tags):
							token: Token = token
							...
							eval_line = "{}\t{}\t{}\t{}\n".format(
								token.text,
								str(tags[0].value)+'_'+str(tags[0].score),
								str(tags[1].value)+'_'+str(tags[1].score),
								str(tags[2].value)+'_'+str(tags[2].score),
							)
							nbest_lines.append(eval_line)
						nbest_lines.append("\n")
			...
			if post_process:
				...
				if out_path is not None:
					with open(out_path, "w", encoding="utf-8") as outfile:
						outfile.write("".join(nbest_lines))

				diction, words = self.decode(out_path)
				self.write_to_conllu(diction, "final.conllu")
				# pdb.set_trace()
				precision, recall, acc, seq_acc = self.eval_conllu('final.conllu', 'data/ptb_3.3.0_modified/test.conllu')
				up, ur, uf1, _ = self.eval_conllu('final.conllu', 'data/ptb_3.3.0_modified/test.conllu', labeled=False)
				log.info(
					f"TEST_CONNLU UNLABELED ACC: score {uf1}"
				)
				log.info(
					f"TEST_CONNLU LABELED ACC: score {acc}"
				)
			else:
				if out_path is not None:
					with open(out_path, "w", encoding="utf-8") as outfile:
	...
	def _obtain_labels(
			self, feature, sentences, get_nbest_tags: bool = True
	) -> (List[List[Label]], List[List[List[Label]]]):
		"""
		Returns a tuple of two lists:
		 - The first list corresponds to the most likely `Label` per token in each sentence.
		 - The second list contains the n best probability of n best `Labels` for each token
		   in a sentence for all sentences.
		"""
		...
		for i, vals in enumerate(zip(feature, lengths)):
			...
			if self.use_crf:
				confidences, tag_seq, scores = self._viterbi_decode(
					feats[:length], all_scores=get_nbest_tags, current_idx=i,
				)
			else:
				tag_seq = []
				confidences = []
			...
			if get_nbest_tags:
				nbest_tags = []
				for score_dist in scores:
					probs, tag_idx = torch.topk(score_dist, k=3, sorted=True)
					nbest_tags.append([Label(self.tag_dictionary.get_item_for_index(tag_idx[i]),float(probs[i])) for i in range(0,len(probs))])
				all_tags.append(
					nbest_tags
				)
	...
	def decode(self, file_with_predicted_labels):
		"""
			...
			if not label_for_each_word == '\n':
				for nb_label in range(1, 4):
					...
					word_details = {
						...
						6: relation,
						7: proc}
					decoded.update({index_of_word: word_details})
				...
				if self.check_if_multiple_roots(decoded_words):
					self.choose_root_from_multiple_enc1_2(
				...
				self.get_rid_of_multiple_roots(decoded_words)
				# pdb.set_trace()
				# attach nodes without heads to the single root
				self.assign_homeless_nodes_to_root(
					decoded_words, homeless_nodes)
				# pdb.set_trace()
				# attach first node involved in a cycle to the single root
				self.find_cycles(decoded_words, decoded_sentence)
				# pdb.set_trace()
				self.prepare_conll_format(decoded_words)
		...
		nb_words = self.count_nb_words(sent_to_write_to_file)


		to_decode.close()
		return sent_to_write_to_file, nb_words

	def decode_2(self, decoded_sentence, decoded_words, homeless_nodes):
		for index_of_word in decoded_sentence:
			if not index_of_word == 0:
				info_about_word = decoded_sentence[index_of_word]
				predicted_rel_position_head = int(
					info_about_word[5]) + index_of_word

				if 0 < predicted_rel_position_head < len(decoded_sentence):
					# word_full_info_ 1:index 2:token 3:lemma 4:postag
					# 5:rel_position_head 6:rel
					words_full_info = {
						1: index_of_word,
						2: info_about_word[2],
						3: info_about_word[3],
						4: info_about_word[4],
						5: predicted_rel_position_head,
						6: info_about_word[6]}
					decoded_words.update({index_of_word: words_full_info})
				elif predicted_rel_position_head == 0 and info_about_word[6] == "root":
					words_full_info = {
						1: index_of_word,
						2: info_about_word[2],
						3: info_about_word[3],
	...
	def check_if_any_root(self, decoded_words):
	...
	def check_if_multiple_roots(self, decoded_words):
		count = 0
	...
	def find_candidates_for_root(self, all_probs, decoded_words, homeless_nodes):
		candidates_for_root = {}
	...
	def assign_root_to_index_one(self, decoded_words, homeless_nodes):
	...
	def choose_root_from_multiple_enc1_2(self,
			decoded_sentence,
			decoded_sentence2,
			decoded_words):
		count = 0
		...
		if most_probably:
			try:
				highest = max(most_probably.items(), key=operator.itemgetter(1))[0]
			except:
				pdb.set_trace()
	...
	def get_rid_of_multiple_roots(self, decoded_words):
		count = 0
		while count > 1:
			count = 0
			for d in decoded_words:
	...
	def assign_homeless_nodes_to_root(self, decoded_words, homeless_nodes):
		root_ind = 0
		for d in decoded_words:
			node = decoded_words[d]
			if node[5] == 0:
	...
	def find_cycles(self, decoded_words, decoded_sentence):
		more_cycles = True
	...
	def prepare_conll_format(self, decoded_words):
	...
	def count_nb_words(self, sent_to_write_to_file):
		nb_words = 0
	...
	def write_to_conllu(self, dict_with_sent, final_file):
	...
	def eval_conllu(self, gold_file, sys_file, labeled=True):
		correct = 0
		predicted = 0
		actual = 0
		n_tokens = 0
		...
		with codecs.open(gold_file, encoding='utf-8') as gf, \
			...
			while gold_line:
				...
				if gold_line.rstrip() != '':
					...
					if labeled:
						gold_edges = set([tuple([gold_head, gold_label])])
					else:
						gold_edges = set([gold_head])
					...
					if labeled:
						sys_edges = set([tuple([sys_head, sys_label])])
					else:
						sys_edges = set([sys_head])
