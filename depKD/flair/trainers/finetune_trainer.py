...
class ModelFinetuner(ModelDistiller):
				...
				for batch_no, student_input in enumerate(batch_loader):
					...
					try:
						if self.distill_mode:
							...
							loss = self.model.simple_forward_distillation_loss(student_input, interpolation = interpolation)
						else:
							if self.n_gpu>1:
	...
	def final_test(
		...
		if overall_test:
			loader=ColumnDataLoader(list(self.corpus.test),eval_mini_batch_size, use_bert=self.use_bert,tokenizer=self.bert_tokenizer, word_map = self.model.word_map, char_map = self.model.char_map, sentence_level_batch = self.sentence_level_batch)
			loader.assign_tags(self.model.tag_type,self.model.tag_dictionary)
			test_results, test_loss = self.model.evaluate(
				loader,
				out_path=base_path / "bllip.60.100000.tsv",
