...
class ConfigParser:
		...
		for corpus in self.corpus_list:
			if is_professor:
				...
				try:
					...
					config = Params.from_file(self.config[self.target]['teachers'])
			config = ConfigParser(config)
			self.teacher_config = config
			teacher_model=self.create_model(config.config, pretrained=True)
	...
	def get_corpus(self):
		...
		for corpus in self.config[self.target]['Corpus'].split(':'):
			...
			elif 'PTB_SL' in corpus:
				if 'train_file' in self.config[self.tag_type]:
					current_dataset=getattr(datasets,corpus)(train_file=self.config[self.tag_type]['train_file'],
					                                         dev_file=self.config[self.tag_type]['dev_file'],
					                                         test_file=self.config[self.tag_type]['test_file'])
					# pdb.set_trace()
				else:
					current_dataset=getattr(datasets,corpus)()
			# =================================For dependency parsing as sequence labeling==============================
